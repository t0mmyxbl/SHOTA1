//THE SNAIL TRAIL GAME - Version 1.0 - Instrumented Version

// This version 1.0 is used for:
// - saving timing data to a file, 'SnailTrailTimes.csv', not just the screen.
// - producing averages for the various timing data.
// - saving key presses to a 'SnailTrailMoves.csv' file for subsequent automated game play (see version 2.0).

// Other changes in this version:
// (1) - Extra boolean (below) to decide whether to record details of each step played... 

bool record_Every_Step(true);						//NEW 'true' = record all details, 'false' = summary only
...
int __cdecl main()
{
	...
	srand(999);								//NEW DEMO seed	-de-randomises the sequence of game element generation					
...
{ //NEW open the files that will store the times produced and the file with the moves recorded.
  //NEW NB. Files are closed automatically by system when program exits (not good but sufficient here)
	...
	ST_Moves.open("SnailTrailMoves.csv", ios::app);		// open for writing moves to initially
